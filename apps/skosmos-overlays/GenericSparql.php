...
EOF;
    ...
    /**
     * Build VALUES (?prop ?pri ?langParam) for concept search.
     ...
     * @return string SPARQL VALUES clause
     */
    protected function generateConceptSearchPropertyValues($props, $langClause)
    {
        $priorities = array(
            'skos:prefLabel' => 1,
            'skos:altLabel' => 3,
        ...
        $tuples = array();
        foreach ($props as $prop) {
            $pri = array_key_exists($prop, $priorities) ? $priorities[$prop] : 9;
            $langParam = ($prop === 'skos:notation') ? "''" : $langClause;
            $tuples[] = "($prop $pri $langParam)";
        }

        return 'VALUES (?prop ?pri ?langParam) { ' . implode(' ', $tuples) . ' }';
    }
...
EOQ;
    ...
    *  This function can be overwritten in other SPARQL dialects for the possibility of handling the different language clauses
    ...
    protected function generateConceptSearchQuery($fields, $unique, $params, $showDeprecated = false)
    {
        ...
        //also merge vocabulary-specific skosmos:searchProperty values (e.g. xkos notes)
        if ($vocabs) {
            ...
            foreach ($vocabs as $vocab) {
                ...
                foreach ($vocab->getConfig()->getSearchProperties() as $searchProp) {
                    if (!in_array($searchProp, $props, true)) {
                        $props[] = $searchProp;
                    }
                }
            }
